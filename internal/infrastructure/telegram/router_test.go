...
func setupRouter() *routerFixture {
	...
	return &routerFixture{
		...
		intentParser: intentParser, cardManager: cardManager,
	}
}
...
func TestRouter_Message_ShowsPreviewWithKeyboard(t *testing.T) {
	...
	f.intentParser.On("ParseIntent", mock.Anything, "Buy milk").Return(&dto.IntentOutput{
		Action: "create_task",
		Title:  "Buy milk",
	}, nil)
	f.api.On("Send", mock.Anything).Return(nil)
...
func TestRouter_Message_NonCreateAction(t *testing.T) {
	f := setupRouter()
	user := entity.NewUser(valueobject.TelegramID(100))
	user.SetDefaultBoard("b1")
	user.SetDefaultList("l1")
	...
	f.intentParser.On("ParseIntent", mock.Anything, "what lists are on the board?").Return(&dto.IntentOutput{
		Action: "list_lists",
	}, nil)
	f.board.On("GetLists", mock.Anything, "tok", "b1").Return([]port.ListInfo{
		{ID: "l1", Name: "To Do"},
		{ID: "l2", Name: "Done"},
	}, nil)
	f.api.On("Send", mock.Anything).Return(nil)

	f.router.Route(f.api, makeMessage(100, 200, "what lists are on the board?"))

	f.api.AssertCalled(t, "Send", mock.MatchedBy(func(c tgbotapi.Chattable) bool {
		msg, ok := c.(tgbotapi.MessageConfig)
		return ok && assert.Contains(t, msg.Text, "Board Lists")
	}))
}
...
func TestRouter_CallbackConfirmCreate(t *testing.T) {
	...
	// Parse first to store pending task (now uses intent-based flow)
	f.intentParser.On("ParseIntent", mock.Anything, "Buy milk").Return(&dto.IntentOutput{
		Action: "create_task",
		Title:  "Buy milk",
	}, nil)
